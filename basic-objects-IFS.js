...
function tetraedre(side) {
   ...
   // face de base 
   face( [-s,0,0,s,0,0,0,0,s], [0,-1,0]);
   // face gauche 
   face( [0,0,s,0,s,0,-s,0,0], normalize(cross(vec3(1,1,0),vec3(0,1,0))));
   //face droite
   face( [0,0,s,s,0,0,0,s,0], normalize(cross(vec3(1,0,-1),vec3(0,1,-1))));
   //face arriere
   face( [-s,0,0,0,s,0,s,0,0], [0,0,-1]);
...
function triangle(side) {
   ...
   face( [0,0,0,0,0,s,s,0,s,s,0,0], [0,-1,0]); // base 
   face( [0,0,0,0,0,s,0,s,0], [-1,0,0] ); // face gauche triangle
   face( [s,0,0,s,s,0,s,0,s], [1,0,0] ); // face droite triangle 
   face( [0,0,0,0,s,0,s,s,0,s,0,0,], [0,0,-1] ); // face base arriere
   face( [0,s,0,0,0,s,s,0,s,s,s,0], normalize(cross(vec3(1,0,0),vec3(0,1,-1))) ); // face avant 
...
function Reacteur(radius, height, slices, noTop, noBottom) {
   radius = radius || 0.5;
   height = height || 2*radius;
   slices = slices || 32;
   var vertexCount = 2*(slices+1);
   ...
   for (i = 0; i <= slices; i++) {
      ...
      vertices[kv] = c*radius*1.5;
      normals[kv++] = c;
      // On multiplie par 1.5 
      vertices[kv] = s*radius*1.5;
      normals[kv++] = s;
      vertices[kv] = -height/2;
      normals[kv++] = 0;
      texCoords[kt++] = i/slices;
